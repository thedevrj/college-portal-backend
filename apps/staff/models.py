...
class Staff(models.Model):
    ...
    department_section_cell = models.CharField(max_length=255, blank=True, verbose_name="Department/Section/Cell")
    roles = models.CharField(max_length=500, blank=True, help_text="Comma separated values indicating associated sections for roles")
    ...
    staff_type = models.CharField(max_length=100, blank=True, help_text="e.g., Permanent, Contract")
        ...
        if self.staff_no:
            try:
                ...
                if Faculty.objects.filter(staff_no=self.staff_no).exists():
                    raise ValidationError({"staff_no": "A Faculty member with this staff number already exists."})
            except LookupError:
                pass
    ...
    def save(self, *args, **kwargs):
        ...
        while True:
            collision = Staff.objects.filter(slug=unique_slug).exclude(pk=self.pk).first()
            if not collision:
                break
